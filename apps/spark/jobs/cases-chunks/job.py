...
def _get_text_model():
    ...
    if _text_model is None:
        ...
        _text_model = SentenceTransformer("rufimelo/Legal-BERTimbau-base")
    return _text_model
...
def chunk_markdown(df: DataFrame) -> DataFrame:
    ...
    df_chunks = df_chunks.withColumn("embedding_model", F.lit("rufimelo/Legal-BERTimbau-base"))
    print("Generating text embeddings (rufimelo/Legal-BERTimbau-base, 768d)...")
...
def main():
    ...
    doc_ids_env = os.environ.get("GCS_MD_DOC_IDS", "")
    filter_doc_ids = (
        {int(x.strip()) for x in doc_ids_env.split(",") if x.strip()}
        if doc_ids_env else None
    )
    lance_mode = os.environ.get("LANCE_SINK_MODE", "overwrite")
    ...
    sink_opts = LanceSinkOptions(
        ...
        mode=lance_mode,
        indexes=[
            VectorIndexSpec(index_type="IVF_PQ", column="text_embedding", dimension=768, num_partitions=4, num_sub_vectors=16),
    ...
    if filter_doc_ids:
        df = df.filter(col("doc_id").isin(list(filter_doc_ids)))
